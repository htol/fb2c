...
- **On-device A/B probes** (verified 2026-08-18): use `make kindle-probe SUFFIX=x [INPUT=fixture.fb2]` — it rewrites `<book-title>` to "fb2c probe x" (unique shelf title AND unique ASIN, which fb2c derives as UUIDv5 of title+authors; the device dedups by ASIN) and purges every trace of previous probes: `documents/fb2c_probe_*` + `.sdr`, 0-byte `system/thumbnails/*.tmp.partial` corpses, `system/bookcovers/*/*.jpg` renders. Without the purge the device stitches a fresh probe onto stale shelf state — observed: a never-opened probe showed "3% read", no NEW ribbon, and no thumbnail regeneration ran
- **Two cover stores on-device** (verified 2026-08-18, two different e-ink Kindles): `system/thumbnails/thumbnail_<id>_EBOK_portrait.jpg` (or `thumbnail_<numeric-id>.jpg`) feeds the shelf grid; `system/bookcovers/<device-id>/<uuid>_<ASIN>_EBOK.jpg` is a rendered cover cache that exists only on some devices/firmware (present on one test device, absent on another — absence does not break tiles). A 0-byte `.tmp.partial` in thumbnails/ means the shelf step died
- **cdeType=EBOK kills shelf thumbnails when paired with an ASIN** (verified on-device 2026-08-18, probes B/C/D–G on two devices): EXTH 501=`EBOK` + EXTH 113 routes the firmware into its "store book" path, where the thumbnail generator dies on an unvalidatable ASIN and leaves a 0-byte `thumbnail_<ASIN>_EBOK_portrait.jpg.tmp.partial`. Any other combination — no 501, or 501=`PDOC`, with or without ASIN — takes the personal-document path: the device assigns its own numeric pseudo-ASIN (ignoring EXTH 113 for the thumbnail name) and renders the tile from the EXTH 201 cover. fb2c therefore writes `PDOC`. This also explains Calibre: versions writing `PDOC` thumbnail fine; a Calibre 9.12 file (writes `EBOK`+UUID) died the same 0-byte death
- **Dependencies**: Go 1.26.6, pinned via `toolchain` in go.mod — the stdlib `image/jpeg` encoder (cover/thumbnail re-encode) produces different bytes across Go minor versions, which breaks byte goldens; external tools (Calibre, mobitool) optional for validation
